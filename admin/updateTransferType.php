...
try {
    ...
    $sql = "UPDATE transfer_types 
            SET type_key = ?, name = ?, description = ?, is_round_trip_option = ?, max_passengers = ?
            WHERE id = ?";
    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('sssiii', $typeKey, $name, $description, $isRoundTripOption, $maxPassengers, $id);
    ...
    if ($stmt->execute()) {
        ...
        throw new Exception('Failed to update transfer type');
    }
